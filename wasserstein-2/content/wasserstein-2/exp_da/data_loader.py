...
def get_mnist_loader(config, batch_size, train=True):
    # Deprecated, transforms.Scale -> transforms.Resize ZMALIK 20230308
    #tf = transforms.Compose([transforms.Scale(16),
    #                         transforms.CenterCrop(16),
    #                         transforms.ToTensor(),
    #                         transforms.Lambda(lambda x: 2*x-1)])
    tf = transforms.Compose([transforms.Resize(16),
                             transforms.CenterCrop(16),
                             transforms.ToTensor(),
                             transforms.Lambda(lambda x: 2*x-1)])
...
def get_usps_loader(config, batch_size, train=True):
    tf = transforms.Compose([transforms.ToTensor(),
                             transforms.Lambda(lambda x: 2*x-1)])
    # This requires us to have already downloaded USPS dataset, instead use
    # built in torchvision USPS dataset.
    #usps = USPS(root=config.usps_path, train=train, transform=tf)
    usps = datasets.USPS(root = config.usps_path, train = train, download = True, transform = tf)
